export const onpuSlide = () => {
  ...
  parts.forEach((part) => {
    // 右にスライドする音符にクラスをつける
    const tamas = [
      ...part.querySelectorAll(".js-onpuslide-one"),
    ].reverse() as HTMLElement[]; //次の要素にis-slideがあるかどうかを判定するところがあるので、後ろの要素から処理してis-slideをつけておく
    ...
    const line3 = part.querySelector(
      ".onpuLine-item.is-bottom1"
    ) as HTMLElement;
    const line4 = part.querySelector(
      ".onpuLine-item.is-bottom2"
    ) as HTMLElement;
